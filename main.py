...
# --- 2. KIỂM TRA VÀ KHỞI TẠO DỮ LIỆU BẢN ĐỒ (PHƯƠNG PHÁP CUỐI CÙNG) ---
inspector = inspect(engine)
if not inspector.has_table('nodes'):
    print("Database is empty. Initializing map data from OpenStreetMap...")
    
    # Define a bounding box of coordinates around Bach Khoa, Hanoi
    # North, South, East, West
    north, south, east, west = 21.01, 20.99, 105.85, 105.83
    
    # Download the map data directly from the bounding box
    print("Downloading graph from bounding box...")
    G_temp = ox.graph_from_bbox(north, south, east, west, network_type='drive')
    
    # Convert and save to the database (same as before)
